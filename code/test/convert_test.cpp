...
TEST(convert, convert_and_convert_back_gives_equal_boost_graph) {
  for (int i = 1; i < 100; i++) {
    ...
    int seed = 42 + i;
    // always at least one site but not more deliveries than the model can handle
    int sites_count = i % (N_DELIVERIES - 1) + 1;
    generate(sites_count, seed, s_t_g);
    auto graph = convert_to_graph(s_t_g.graph);
    auto boost_graph = convert_to_boost_graph(graph);
    ASSERT_TRUE(equal_boost_graphs(s_t_g.graph, boost_graph));
  }
}
